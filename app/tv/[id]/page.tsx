...
import { useTVShowDetails } from "@/hooks/use-tmdb"
...
export default function TVShowPage() {
  ...
  const { tvShow, similarTVShows, loading, error } = useTVShowDetails(tvId)
  ...
  const handlePlayClick = () => {
    if (tvShow) {
      ...
      const id = getFallbackTrailerId(tvShow.title)
      setTrailerId(id)
      setIsVideoOpen(true)
    }
  }
  ...
  const handleLikeClick = () => {
    if (tvShow) {
      toggleLike(tvShow.id)
    }
  }

  const inMyList = tvShow ? isInMyList(tvShow.id) : false
  const liked = tvShow ? isLiked(tvShow.id) : false
      ...
      <main className="relative min-h-screen">
        ...
        <div className="relative h-[70vh] w-full">
          <div className="absolute inset-0">
            <Image
              src={tvShow.bannerUrl || tvShow.imageUrl}
              alt={tvShow.title}
              fill
        ...
        <div className="relative -mt-60 md:-mt-80 px-4 md:px-12 max-w-6xl mx-auto">
          <h1 className="text-4xl md:text-6xl font-bold mb-4">{tvShow.title}</h1>

          <div className="flex items-center gap-4 mb-6">
            <span className="text-green-500 font-semibold">{Math.round(tvShow.rating * 10)}% Match</span>
            <span>{tvShow.releaseYear}</span>
            <span className="border border-white/40 px-2 py-0.5 text-sm">SERIE</span>
          ...
          <div className="grid md:grid-cols-3 gap-8">
            <div className="md:col-span-2">
              <p className="text-lg mb-6">{tvShow.description}</p>
            </div>
            <div>
              ...
              <p className="text-white/60 mb-1">
                <span className="text-white/90">Temporadas: </span>
                {tvShow.seasons}
              </p>
            </div>
          </div>
        ...
        <div className="mt-16">
          <MovieRow title="Series similares" movies={similarTVShows} isLoading={loading} />
        </div>
      </main>
